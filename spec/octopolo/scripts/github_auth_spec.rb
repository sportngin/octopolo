    ...
    describe GithubAuth do
      ...
      context "#execute" do
        ...
        it "asks for username and password when that option is selected, generates the token, and writes to disk" do
          expect(subject).to receive(:ask_auth_method).and_return("Generate an API token with my credentials")
          expect(subject).to receive(:ask_credentials)
          expect(subject).to receive(:request_token)
          expect(subject).to receive(:store_token)

          expect(subject).not_to receive(:ask_token)
        ...
        it "gracefully handles invalid credentials" do
          expect(subject).to receive(:ask_auth_method).and_return("Generate an API token with my credentials")
          expect(subject).to receive(:ask_credentials)
          expect(subject).to receive(:request_token).and_raise(GitHub::BadCredentials.new(bad_credentials_message))
          expect(subject).not_to receive(:store_token)
          expect(cli).to receive(:say).with(bad_credentials_message)

          expect { subject.execute }.to_not raise_error
        end

        it "asks for a token when that option is selected, and writes to disk" do
          expect(subject).to receive(:ask_auth_method).and_return("I'll enter an access token manually")
          expect(subject).to receive(:ask_token)
          expect(subject).to receive(:verify_token)
          expect(subject).to receive(:store_token)

          expect(subject).not_to receive(:ask_credentials)
        ...
        it "gracefully handles an invalid token" do
          expect(subject).to receive(:ask_auth_method).and_return("I'll enter an access token manually")
          expect(subject).to receive(:ask_token)
          expect(subject).to receive(:verify_token).and_raise(GitHub::BadCredentials.new(bad_credentials_message))
          expect(subject).not_to receive(:store_token)
          expect(cli).to receive(:say).with(bad_credentials_message)
      ...
      context "#ask_credentials" do
        it "asks for and captures the user's GitHub credentials" do
          expect(cli).to receive(:prompt).with("Your GitHub username: ") { username }
          expect(cli).to receive(:prompt_secret).with("Your GitHub password (never stored): ") { password }
      ...
      context "#ask_token" do
        it "asks for and captures the user's manually entered access token" do
          expect(cli).to receive(:prompt).with("Your GitHub username: ") { username }
          expect(cli).to receive(:prompt_secret).with("Your GitHub API token: ") { user_defined_token }
      ...
      context "#request_token" do
        ...
        it "requests an auth token from GitHub's API and captures the HTTP response" do
          expect(cli).to receive(:perform_quietly).with(%Q(curl -u '#{username}:#{password}' -d '{"scopes": ["repo"], "notes": "Octopolo"}' https://api.github.com/authorizations)) { json_response }
      ...
      context "#verify_token" do
        ...
        it "verifies that a user_defined_token properly authenticates with the GitHub API" do
          expect(cli).to receive(:perform_quietly).with(%Q(curl -u #{user_defined_token}:x-oauth-basic https://api.github.com/user)) { json_response }
      ...
      context "#store_token" do
        ...
        it "stores the username and token if receiving a token from GitHub" do
          ...
          expect(user_config).to receive(:set).with(:github_user, username)
          expect(user_config).to receive(:set).with(:github_token, token)
          expect(cli).to receive(:say).with("Successfully stored GitHub API token.")
        ...
        it "stores the token if a token has been manually entered" do
          ...
          subject.user_defined_token = user_defined_token
          expect(user_config).to receive(:set).with(:github_user, username)
          expect(user_config).to receive(:set).with(:github_token, user_defined_token)
          expect(cli).to receive(:say).with("Successfully stored GitHub API token.")
          subject.send(:store_token)
        end
      end
