  ...
  describe DatedBranchCreator do
    ...
    context "#perform" do
      it "creates the branch and handles cleaning up and posting about it" do
        ...
        subject.should_receive(:post_about_branch)
    ...
    context "#post_about_branch" do
      it "correctly posts for deployable branches" do
        subject.branch_type = Git::DEPLOYABLE_PREFIX
        Zapier::BranchPoster.should_receive(:perform).with(config.app_name, subject.branch_name, Git::DEPLOYABLE_PREFIX)
        subject.post_about_branch
      end

      it "correctly posts for staging branches" do
        subject.branch_type = Git::STAGING_PREFIX
        Zapier::BranchPoster.should_receive(:perform).with(config.app_name, subject.branch_name, Git::STAGING_PREFIX)
        subject.post_about_branch
      end

      it "raises an exception for any other branch type" do
        subject.branch_type = "asdfasdfasdf"
        expect { subject.post_about_branch }.to raise_error(DatedBranchCreator::InvalidBranchType, "'#{subject.branch_type}' is not a valid branch type")
      end
    end
  end
end
